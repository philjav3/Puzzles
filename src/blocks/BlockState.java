...
public class BlockState extends SimpleVertex implements State {
    ...
    public int getHeuristic(State goal) {
        ...
        Stack<Character> goalP = finalState.getP();
        Stack<Character> goalQ = finalState.getQ();
        Stack<Character> goalR = finalState.getR();
        ...
        for (int i = 0; i < size; i++) {
            for (int pi = 0; i < p.size(); pi++) {
                if (p.elementAt(pi) != goalP.elementAt(i)) {
                    sum += 1;
                }
            }
            for (int qi = 0; i < q.size(); qi++) {
                if (p.elementAt(qi) != goalQ.elementAt(i)) {
                    sum += 1;
                }
            }
            for (int ri = 0; i < r.size(); ri++) {
                if (p.elementAt(ri) != goalR.elementAt(i)) {
                    sum += 1;
                }
            }
        }
